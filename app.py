...
# Inicializácia Google Auth (číta údaje zo st.secrets)
auth = Authenticate(
...
# Skontroluje, či sme sa práve vrátili z Google prihlásenia
...
if st.session_state.get('connected'):
    ...
    st.markdown(f"""
        <div style="background:white; padding:30px; border-radius:30px; border:1px solid #F1F5F9; width:100%;">
            <img src="{user.get('picture')}" style="width:80px; border-radius:50%; margin-bottom:15px; border: 3px solid #F1F5F9;">
            <h2 style="margin:0; color:#1E293B;">{user.get('name')}</h2>
    ...
    # Google tlačidlo (automaticky vygenerované knižnicou)
